...
from chat_history import generate_session_id, save_chat_history, load_chat_history
from config import (
...
pdf_hash_path = HASHES_FOLDER / PDF_HASH_FILE
...
top_k = st.slider(
    "Number of reference pages to use",
    min_value=1,
    max_value=6,
    value=3,
    help="Select how many pages from the reports should be retrieved to answer the question.",
)
...
if question and st.button("💬 Get Answer"):
    with st.container():
        ...
        try:
            img_paths = search_image_by_question(question, co, top_k=top_k)
...
def get_image_base64(image_path):
    ...
    if not resolved_path.exists():
        st.warning(f"⚠️ Asset not found: {resolved_path}")
        return ""
    with resolved_path.open("rb") as img_file:
        return base64.b64encode(img_file.read()).decode()


user_icon = get_image_base64(USER_AVATAR_PATH)
bot_icon = get_image_base64(BOT_AVATAR_PATH)
...
for idx, pair in enumerate(reversed(st.session_state.chat_history)):
    if pair.get("images"):
        ...
        for img_path in pair["images"]:
            resolved_img = _resolve_path(img_path)
            if resolved_img.exists():
                img_name = resolved_img.name
                with resolved_img.open("rb") as img_file:
